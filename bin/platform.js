 ...
 * @returns {string} Package name like "kajiflow-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `kajiflow-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "kajiflow-darwin-arm64/bin/kajiflow"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/kajiflow${ext}`;
}
