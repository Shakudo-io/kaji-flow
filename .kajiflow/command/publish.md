---
description: Publish kajiflow to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for kajiflow. Execute the FULL publish workflow from start to finish.
...
for PLATFORM in $PLATFORMS; do
  npm view "kajiflow-${PLATFORM}" version
done
```
...
| Package | Description |
|---------|-------------|
| `kajiflow-darwin-arm64` | macOS Apple Silicon |
| `kajiflow-darwin-x64` | macOS Intel |
| `kajiflow-linux-x64` | Linux x64 (glibc) |
| `kajiflow-linux-arm64` | Linux ARM64 (glibc) |
| `kajiflow-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `kajiflow-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
| `kajiflow-windows-x64` | Windows x64 |
...
- New version number
- GitHub release URL: https://github.com/code-yeongyu/kajiflow/releases/tag/v{version}
- npm package URL: https://www.npmjs.com/package/kajiflow
...
<current-context>
<published-version>
!`npm view kajiflow version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view kajiflow version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
