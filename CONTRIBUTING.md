# Contributing to KajiFlow

First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to kajiflow.
...
git clone https://github.com/code-yeongyu/kajiflow.git
cd kajiflow
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   {
     "plugin": [
       "file:///absolute/path/to/kajiflow/dist/index.js"
     ]
   }
   ```
   
   For example, if your project is at `/Users/yourname/projects/kajiflow`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/kajiflow/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"kajiflow"` from the plugin array if it exists, to avoid conflicts with the npm version.
...
│   └── index.ts       # Main plugin entry (KajiFlowPlugin)
...
Thank you for contributing to KajiFlow! Your efforts help make AI-assisted coding better for everyone.
