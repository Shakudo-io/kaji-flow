...
describe("external-plugin-detector", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kaji-test-"))
  })
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only kajiflow is configured", () => {
      // given - opencode.json with only kajiflow
      const opencodeDir = path.join(tempDir, ".opencode")
      fs.mkdirSync(opencodeDir, { recursive: true })
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kajiflow"] })
      )
      ...
      expect(result.detected).toBe(false)
      expect(result.pluginName).toBeNull()
      expect(result.allPlugins).toContain("kajiflow")
    })
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kajiflow", "opencode-notifier"] })
      )
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kajiflow", "opencode-notifier@1.2.3"] })
      )
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kajiflow", "@mohak34/opencode-notifier"] })
      )
