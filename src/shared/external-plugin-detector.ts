/**
 * Detects external plugins that may conflict with kajiflow features.
...
/**
 * Known notification plugins that conflict with kajiflow's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[kajiflow] External notification plugin detected: ${pluginName}

Both kajiflow and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   kajiflow's session-notification has been auto-disabled.

   To use kajiflow's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in kajiflow.json`
}
