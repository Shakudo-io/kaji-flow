...
describe("createSisyphusJuniorAgentWithOverrides", () => {
  ...
  describe("tool safety (task blocked, call_kaji_agent allowed)", () => {
    test("task remains blocked, call_kaji_agent is allowed via tools format", () => {
      // given
      const override = {
        tools: {
          task: true,
      ...
      if (tools) {
        ...
        // call_kaji_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(tools.call_kaji_agent).toBe(true)
        expect(tools.read).toBe(true)
      }
      if (permission) {
        expect(permission.task).toBe("deny")
        // call_kaji_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(permission.call_kaji_agent).toBe("allow")
      }
    })
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // then - task blocked, but call_kaji_agent allowed for explore/librarian spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
