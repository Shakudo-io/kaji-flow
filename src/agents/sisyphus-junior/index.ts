...
// Note: call_kaji_agent is ALLOWED so subagents can spawn explore/librarian
...
export function createSisyphusJuniorAgentWithOverrides(
  ...
  merged.call_kaji_agent = "allow"
  const toolsConfig = { permission: { ...merged, ...basePermission } }

  const base: AgentConfig = {
    description: override?.description ??
      "Focused task executor. Same discipline, no delegation. (Sisyphus-Junior - KajiFlow)",
    mode: MODE,
    model,
    temperature,
    maxTokens: 64000,
