...
describe("createBuiltinAgents with model overrides", () => {
  ...
  test("Atlas uses uiSelectedModel when provided", async () => {
    ...
    try {
      ...
      // #then
      expect(agents["senior-orchestrator"]).toBeDefined()
      expect(agents["senior-orchestrator"].model).toBe("openai/gpt-5.2")
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  test("user config model takes priority over uiSelectedModel for senior-orchestrator", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["openai/gpt-5.2", "anthropic/claude-sonnet-4-5"])
    )
    ...
    try {
      ...
      // #then
      expect(agents["senior-orchestrator"]).toBeDefined()
      expect(agents["senior-orchestrator"].model).toBe("google/antigravity-claude-opus-4-5-thinking")
    } finally {
      fetchSpy.mockRestore()
    }
  })
...
describe("createBuiltinAgents with requiresProvider gating (developer)", () => {
  ...
  test("developer is created when github-copilot provider is connected", async () => {
  ...
  test("developer is created on first run when no availableModels or cache exist", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.developer).toBeDefined()
      expect(agents.developer.model).toBe("openai/gpt-5.3-codex")
    } finally {
      cacheSpy.mockRestore()
      fetchSpy.mockRestore()
    }
  ...
  test("developer is created when explicit config provided even if provider unavailable", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["anthropic/claude-opus-4-6"])
    )
    const overrides = {
      developer: { model: "anthropic/claude-opus-4-6" },
    }
...
describe("createBuiltinAgents with requiresAnyModel gating (orchestrator)", () => {
  ...
  test("orchestrator is not created when no fallback model is available and provider not connected", async () => {
    // #given - only openai/gpt-5.2 available, not in orchestrator fallback chain
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
...
describe("override.category expansion in createBuiltinAgents", () => {
  ...
  test("senior-orchestrator override with category expands category properties", async () => {
    ...
    // #then - ultrabrain category: model=openai/gpt-5.3-codex, variant=xhigh
    expect(agents["senior-orchestrator"]).toBeDefined()
    expect(agents["senior-orchestrator"].model).toBe("openai/gpt-5.3-codex")
    expect(agents["senior-orchestrator"].variant).toBe("xhigh")
  })
...
describe("agent override tools migration", () => {
  test("tools: { x: false } is migrated to permission: { x: deny }", async () => {
    ...
    // #then
    expect(agents["context-finder"]).toBeDefined()
    const permission = agents["context-finder"].permission as Record<string, string>
  ...
  test("tools config is removed after migration", async () => {
    ...
    // #then
    expect(agents["context-finder"]).toBeDefined()
    expect((agents["context-finder"] as any).tools).toBeUndefined()
  })
})
