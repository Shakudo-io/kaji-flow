...
const TEST_STORAGE_ROOT = join(tmpdir(), `senior-orchestrator-message-storage-${randomUUID()}`)
...
const { createSeniorOrchestratorHook } = await import("./index")
const { MESSAGE_STORAGE } = await import("../../features/hook-message-injector")

describe("senior-orchestrator hook", () => {
  let TEST_DIR: string
  ...
  function createMockPluginInput(overrides?: { promptMock?: ReturnType<typeof mock> }) {
    ...
    } as unknown as Parameters<typeof createSeniorOrchestratorHook>[0] & { _promptMock: ReturnType<typeof mock> }
  }
  ...
  beforeEach(() => {
    TEST_DIR = join(tmpdir(), `senior-orchestrator-test-${randomUUID()}`)
    SISYPHUS_DIR = join(TEST_DIR, ".kajiflow/work")
  ...
  describe("tool.execute.after handler", () => {
    test("should handle undefined output gracefully (issue #1035)", async () => {
      ...
      const hook = createSeniorOrchestratorHook(createMockPluginInput())
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should not transform when caller is not SeniorOrchestrator", async () => {
       // given - boulder state exists but caller agent in message storage is not SeniorOrchestrator
     ...
     test("should append standalone verification when no boulder state but caller is SeniorOrchestrator", async () => {
       // given - no boulder state, but caller is SeniorOrchestrator
       const sessionID = "session-no-boulder-test"
       setupMessageStorage(sessionID, "senior-orchestrator")
     ...
     test("should transform output when caller is SeniorOrchestrator with boulder state", async () => {
       // given - SeniorOrchestrator caller with boulder state
     ...
     test("should still transform when plan is complete (shows progress)", async () => {
       // given - boulder state with complete plan, SeniorOrchestrator caller
     ...
     test("should append session ID to boulder state if not present", async () => {
       // given - boulder state without session-append-test, SeniorOrchestrator caller
     ...
     test("should not duplicate existing session ID", async () => {
       // given - boulder state already has session-dup-test, SeniorOrchestrator caller
     ...
     test("should include boulder.json path and notepad path in transformed output", async () => {
       // given - boulder state, SeniorOrchestrator caller
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "senior-orchestrator")
       })
      ...
      test("should append delegation reminder when orchestrator writes outside .kajiflow/work/", async () => {
        // given
        const hook = createSeniorOrchestratorHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .kajiflow/work/", async () => {
        // given
        const hook = createSeniorOrchestratorHook(createMockPluginInput())
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .kajiflow/work/", async () => {
        // given
        const hook = createSeniorOrchestratorHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.kajiflow/work/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .kajiflow/work/", async () => {
        // given
        const nonOrchestratorSession = "non-orchestrator-session"
        setupMessageStorage(nonOrchestratorSession, "orchestrator-junior")
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .kajiflow/work\\ (Windows backslash)", async () => {
          // given
          const hook = createSeniorOrchestratorHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".kajiflow/work\\plans\\work-plan.md" },
          }
        ...
        test("should NOT append reminder when orchestrator writes inside .kajiflow/work with mixed separators", async () => {
          // given
          const hook = createSeniorOrchestratorHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .kajiflow/work\\", async () => {
          // given
          const hook = createSeniorOrchestratorHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: "C:\\Users\\test\\project\\.kajiflow/work\\plans\\x.md" },
          }
        ...
        test("should append reminder for Windows path outside .kajiflow/work\\", async () => {
          // given
          const hook = createSeniorOrchestratorHook(createMockPluginInput())
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       setupMessageStorage(MAIN_SESSION_ID, "senior-orchestrator")
     })
    ...
    test("should inject continuation when boulder has incomplete tasks", async () => {
      ...
      const hook = createSeniorOrchestratorHook(mockInput)
    ...
    test("should include plan progress in continuation prompt", async () => {
     ...
     test("should inject when last agent matches boulder agent even if non-SeniorOrchestrator", async () => {
