...
/**
 * Get the cache directory for kajiflow binaries.
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[kajiflow] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[kajiflow] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[kajiflow] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[kajiflow] Comment checking disabled.`)
    return null
  }
}
