...
import { ALLOWED_SUBAGENTS, CALL_KAJI_AGENT_DESCRIPTION } from "./constants"
...
export function createCallOmoAgent(
  ...
  const agentDescriptions = ALLOWED_SUBAGENTS.map(
    (name) => `- ${name}: Specialized agent for ${name} tasks`
  ).join("\n")
  const description = CALL_KAJI_AGENT_DESCRIPTION.replace("{agents}", agentDescriptions)
  ...
  return tool({
    ...
    args: {
      ...
      subagent_type: tool.schema
        .string()
        .describe("The type of specialized agent to use for this task (context-finder or researcher only)"),
    ...
    async execute(args: CallOmoAgentArgs, toolContext) {
      ...
      log(`[call_kaji_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`)

      // Case-insensitive agent validation - allows "ContextFinder", "EXPLORE", "context-finder" etc.
      if (![...ALLOWED_SUBAGENTS].some(
        (name) => name.toLowerCase() === args.subagent_type.toLowerCase()
      )) {
        return `Error: Invalid agent type "${args.subagent_type}". Only ${ALLOWED_SUBAGENTS.join(", ")} are allowed.`
      }
      
      const normalizedAgent = args.subagent_type.toLowerCase() as typeof ALLOWED_SUBAGENTS[number]
...
async function executeBackground(
  ...
  try {
    ...
    log("[call_kaji_agent] parentAgent resolution", {
...
async function executeSync(
  ...
  if (args.session_id) {
    log(`[call_kaji_agent] Using existing session: ${args.session_id}`)
    const sessionResult = await ctx.client.session.get({
      path: { id: args.session_id },
    })
    if (sessionResult.error) {
      log(`[call_kaji_agent] Session get error:`, sessionResult.error)
      return `Error: Failed to get existing session: ${sessionResult.error}`
    }
    sessionID = args.session_id
  } else {
    log(`[call_kaji_agent] Creating new session with parent: ${toolContext.sessionID}`)
    const parentSession = await ctx.client.session.get({
      path: { id: toolContext.sessionID },
    }).catch((err) => {
      log(`[call_kaji_agent] Failed to get parent session:`, err)
      return null
    })
    log(`[call_kaji_agent] Parent session dir: ${parentSession?.data?.directory}, fallback: ${ctx.directory}`)
    ...
    if (createResult.error) {
      log(`[call_kaji_agent] Session create error:`, createResult.error)
...
Original error: ${createResult.error}`
    ...
    log(`[call_kaji_agent] Created session: ${sessionID}`)
  }
  ...
  log(`[call_kaji_agent] Sending prompt to session ${sessionID}`)
  log(`[call_kaji_agent] Prompt text:`, args.prompt.substring(0, 100))
   ...
   try {
    ...
    log(`[call_kaji_agent] Prompt error:`, errorMessage)
  ...
  log(`[call_kaji_agent] Prompt sent, polling for completion...`)
  ...
  while (Date.now() - pollStart < MAX_POLL_TIME_MS) {
    // Check if aborted
    if (toolContext.abort?.aborted) {
      log(`[call_kaji_agent] Aborted by user`)
      return `Task aborted.\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
    }
    ...
    if (currentMsgCount > 0 && currentMsgCount === lastMsgCount) {
      ...
      if (stablePolls >= STABILITY_REQUIRED) {
        log(`[call_kaji_agent] Session complete, ${currentMsgCount} messages`)
        break
      }
    } else {
      stablePolls = 0
  ...
  if (Date.now() - pollStart >= MAX_POLL_TIME_MS) {
    log(`[call_kaji_agent] Timeout reached`)
  ...
  if (messagesResult.error) {
    log(`[call_kaji_agent] Messages error:`, messagesResult.error)
    return `Error: Failed to get messages: ${messagesResult.error}`
  }

  const messages = messagesResult.data
  log(`[call_kaji_agent] Got ${messages.length} messages`)
  ...
  if (relevantMessages.length === 0) {
    log(`[call_kaji_agent] No assistant or tool messages found`)
    log(`[call_kaji_agent] All messages:`, JSON.stringify(messages, null, 2))
    return `Error: No assistant or tool response found\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
  }

  log(`[call_kaji_agent] Found ${relevantMessages.length} relevant messages`)
  ...
  log(`[call_kaji_agent] Got response, length: ${responseText.length}`)
