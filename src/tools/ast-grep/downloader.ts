...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[kajiflow] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[kajiflow] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[kajiflow] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[kajiflow] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
