...
function printHeader(isUpdate: boolean): void {
  ...
  console.log()
  console.log(color.bgMagenta(color.white(` KajiFlow... ${mode} `)))
  console.log()
}
...
async function runNonTuiInstall(args: InstallArgs): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo("Usage: bunx kajiflow install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>")
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Adding kajiflow plugin...")
  ...
  printStep(step++, totalSteps, "Writing kajiflow configuration...")
  const kajiResult = writeKajiConfig(config)
  if (!kajiResult.success) {
    printError(`Failed: ${kajiResult.error}`)
    return 1
  }
  printSuccess(`Config written ${SYMBOLS.arrow} ${color.dim(kajiResult.configPath)}`)
  ...
  console.log(`  ${color.dim("gh api --silent --method PUT /user/starred/code-yeongyu/kajiflow >/dev/null 2>&1 || true")}`)
  console.log()
  console.log(color.dim("KajiFlow... Enjoy!"))
  console.log()
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  p.intro(color.bgMagenta(color.white(isUpdate ? " KajiFlow... Update " : " KajiFlow... ")))
  ...
  s.start("Adding kajiflow to OpenCode config")
  const pluginResult = await addPluginToOpenCodeConfig(VERSION)
  if (!pluginResult.success) {
  ...
  s.start("Writing kajiflow configuration")
  const kajiResult = writeKajiConfig(config)
  if (!kajiResult.success) {
    s.stop(`Failed to write config: ${kajiResult.error}`)
    p.outro(color.red("Installation failed."))
    return 1
  }
  s.stop(`Config written to ${color.cyan(kajiResult.configPath)}`)
  ...
  p.log.message(`  ${color.dim("gh api --silent --method PUT /user/starred/code-yeongyu/kajiflow >/dev/null 2>&1 || true")}`)

  p.outro(color.green("KajiFlow... Enjoy!"))
