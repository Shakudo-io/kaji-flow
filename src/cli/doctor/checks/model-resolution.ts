...
export function getModelResolutionInfoWithOverrides(config: KajiConfig): ModelResolutionInfo {
  const agents: AgentResolutionInfo[] = Object.entries(AGENT_MODEL_REQUIREMENTS).map(
    ([name, requirement]) => {
...
function buildDetailsArray(info: ModelResolutionInfo, available: AvailableModelsInfo, config: KajiConfig): string[] {
