...
function getKajiConfig(): string {
  return getConfigContext().paths.kajiConfig
}
...
export function generateKajiConfig(installConfig: InstallConfig): Record<string, unknown> {
  return generateModelConfig(installConfig)
}

export function writeKajiConfig(installConfig: InstallConfig): ConfigMergeResult {
  try {
    ensureConfigDir()
  } catch (err) {
    return { success: false, configPath: getConfigDir(), error: formatErrorWithSuggestion(err, "create config directory") }
  ...
  const kajiConfigPath = getKajiConfig()

  try {
    const newConfig = generateKajiConfig(installConfig)

    if (existsSync(kajiConfigPath)) {
      try {
        const stat = statSync(kajiConfigPath)
        const content = readFileSync(kajiConfigPath, "utf-8")

        if (stat.size === 0 || isEmptyOrWhitespace(content)) {
          writeFileSync(kajiConfigPath, JSON.stringify(newConfig, null, 2) + "\n")
          return { success: true, configPath: kajiConfigPath }
        }
        ...
        writeFileSync(kajiConfigPath, JSON.stringify(merged, null, 2) + "\n")
      } catch (parseErr) {
    ...
    return { success: false, configPath: kajiConfigPath, error: formatErrorWithSuggestion(err, "write kajiflow config") }
  }
}
...
function detectProvidersFromKajiConfig(): { hasOpenAI: boolean; hasOpencodeZen: boolean; hasZaiCodingPlan: boolean; hasKimiForCoding: boolean } {
  const kajiConfigPath = getKajiConfig()
  if (!existsSync(kajiConfigPath)) {
  ...
  try {
    const content = readFileSync(kajiConfigPath, "utf-8")
    const kajiConfig = parseJsonc<Record<string, unknown>>(content)
    if (!kajiConfig || typeof kajiConfig !== "object") {
      return { hasOpenAI: true, hasOpencodeZen: true, hasZaiCodingPlan: false, hasKimiForCoding: false }
    }

    const configStr = JSON.stringify(kajiConfig)
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  result.isInstalled = plugins.some((p) => p.startsWith("kajiflow"))
  ...
  const { hasOpenAI, hasOpencodeZen, hasZaiCodingPlan, hasKimiForCoding } = detectProvidersFromKajiConfig()
