...
```jsonc
{
  "$schema": "https://raw.githubusercontent.com/code-yeongyu/kajiflow/master/assets/kajiflow.schema.json",
...
1. `.opencode/kajiflow.json` (project)
2. User config (platform-specific):

| Platform        | User Config Path                                                                                            |
| --------------- | ----------------------------------------------------------------------------------------------------------- |
| **Windows**     | `~/.config/opencode/kajiflow.json` (preferred) or `%APPDATA%\opencode\kajiflow.json` (fallback) |
| **macOS/Linux** | `~/.config/opencode/kajiflow.json`                                                                    |
...
The `kajiflow` configuration file supports JSONC (JSON with Comments):
- Line comments: `// comment`
- Block comments: `/* comment */`
- Trailing commas: `{ "key": "value", }`

When both `kajiflow.jsonc` and `kajiflow.json` files exist, `.jsonc` takes priority.
...
Common Ollama models that work with kajiflow:
...
**Tracking**: https://github.com/code-yeongyu/kajiflow/issues/1124
...
Or disable via `disabled_agents` in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
...
KajiFlow includes built-in skills that provide additional capabilities:
...
Disable built-in skills via `disabled_skills` in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
...
| `compatibility`    | -       | Required kajiflow version compatibility                                           |
...
**Switch providers** via `browser_automation_engine` in `kajiflow.json`:
...
**Enable tmux integration** via `tmux` in `kajiflow.json`:
...
2. When a background agent spawns, KajiFlow creates a new tmux pane
...
```
1. User-configured model (in kajiflow.json)
...
// kajiflow.json (empty categories section)
{}
...
At runtime, KajiFlow uses a 3-step resolution process to determine which model to use for each agent and category. This happens dynamically based on your configuration and available models.
...
1. **Step 1: User Override** — If you specify a model in `kajiflow.json`, use exactly that
...
│   ┌─────────────────────────────────────────────────────────┐   │
│   │ User specified model in kajiflow.json?            │   │
...
Override any agent or category model in `kajiflow.json`:
...
Disable specific built-in hooks via `disabled_hooks` in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
...
Disable specific built-in commands via `disabled_commands` in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
...
Don't want them? Disable via `disabled_mcps` in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
...
KajiFlow adds refactoring tools (rename, code actions).
All OpenCode LSP configs and custom settings (from opencode.json) are supported, plus additional KajiFlow-specific settings.

Add LSP servers via the `lsp` option in `~/.config/opencode/kajiflow.json` or `.opencode/kajiflow.json`:
